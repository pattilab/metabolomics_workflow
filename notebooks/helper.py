...
#ComBat functions taken from https://github.com/brentp/combat.py
...
def runQCNormalization(rawData,filename_worklist,qcCols):
    #get qc and sample column names
    sampleCols = [x for x in rawData.columns.values]
    sampleCols.sort(key=lambda x:filename_worklist[x])

    #separate out qc data from sample data
    qcData = rawData[qcCols]
    sampData = rawData[sampleCols]

    #normalize samples
    corrFactors = []
    filepositions = {}
    for index,row in sampData.iterrows():
        #get mean intensity for all qc samples
        qcMean = np.mean(qcData.loc[index,:].values) 
        if qcMean > 0:
            qcIndex = 0
            #define initial correction factor
            corrFactor = np.mean([qcData.at[index,qcCols[qcIndex]],
                                  qcData.at[index,qcCols[qcIndex+1]]])/qcMean 
            for file,val in zip(rawData.columns.values,row):
                if filename_worklist[file] > filename_worklist[qcCols[qcIndex+1]] and qcIndex + 2 < len(qcCols):
                    qcIndex += 1
                    ...
                    if filename_worklist[qcCols[qcIndex]] in filepositions:
                        filepositions[filename_worklist[qcCols[qcIndex]]].append(corrFactor)
                    else:
                        filepositions[filename_worklist[qcCols[qcIndex]]] = [corrFactor]
                #normalize datapoint
                if corrFactor > 0:
                    rawData.at[index,file] = rawData.at[index,file]/corrFactor
